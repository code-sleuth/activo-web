...
declare_env_variables() {
  ...
  PACKER_IMG_TAG=$(cat ~/activo/workspace/output)
...
initialise_terraform() {
    ...
    pushd /home/circleci/activo-infra/activo/web
        ...
        terraform init -backend-config="path=${TF_VAR_state_path}" -backend-config="project=${TF_VAR_project}" -backend-config="bucket=${TF_VAR_bucket}" -var="web_env_name=${DEPLOYMENT_ENVIRONMENT}" -var="web__disk_image=${PACKER_IMG_TAG}"  -var="web_reserved_env_ip=${RESERVED_IP}"
    popd
}
...
build_infrastructure() {
    ...
    pushd /home/circleci/activo-infra/activo/web
      ...
      if [ "$DEPLOYMENT_ENVIRONMENT" == "production" ]; then
        ...
        -var="web_reserved_env_ip=${RESERVED_IP}" -var="service_account_email=${SERVICE_ACCOUNT_EMAIL}" -var="web_slack_channel=${SLACK_CHANNEL}" -var="web_slack_webhook_url=${SLACK_CHANNEL_HOOK}" -var="user_microservice_api_url=${USER_MICROSERVICE_API_URL}" \
        -var="user_microservice_api_token=${USER_MICROSERVICE_API_TOKEN}"  2>&1 | tee terraform_output.log
      fi
    popd
}
